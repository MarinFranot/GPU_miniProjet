...
int main()
{
    int MODE = 2; // MODE: {1: simple addition, 2:addition avec du J, 3: addition avec du K}
    ...
    fichier << "taille|J|temps_gpu|memoryThroughput(GB/s)|computationThroughput(GOPS/s)" << std::endl;
    fichier << "Ntest = " << Ntest << " | ";
    fichier << "Mode = " << MODE << " | ";
    fichier << "N_thread = " << N_THREAD << std::endl;

    for (int puissance = 7; puissance < 8; puissance++) {
